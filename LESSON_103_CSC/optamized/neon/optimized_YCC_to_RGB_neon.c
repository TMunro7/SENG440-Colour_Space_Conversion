...
// Upsample the 2×2 chroma block into four values
static inline void upsample_neon(
    uint8_t a, uint8_t b, uint8_t c, uint8_t d,
    int16x4_t *out /* lanes 0–3 */
) {
    int16_t vals[4] = {
        a,
        (a + b) >> 1,
...
static void convert_2x2_YCC_block_neon(
    ...
    // widen to 32 bits and bias chroma by –128
    ...
    // clamp to 0–255
    r32 = vmaxq_s32(r32, vdupq_n_s32(0));
    r32 = vminq_s32(r32, vdupq_n_s32(255));
    g32 = vmaxq_s32(g32, vdupq_n_s32(0));
    g32 = vminq_s32(g32, vdupq_n_s32(255));
    b32 = vmaxq_s32(b32, vdupq_n_s32(0));
    b32 = vminq_s32(b32, vdupq_n_s32(255));
    ...
    // scatter lanes
    R[row  ][col]   = (uint8_t)vget_lane_s16(r16, 0);
    R[row  ][col+1] = (uint8_t)vget_lane_s16(r16, 1);
    R[row+1][col]   = (uint8_t)vget_lane_s16(r16, 2);
    R[row+1][col+1] = (uint8_t)vget_lane_s16(r16, 3);

    G[row  ][col]   = (uint8_t)vget_lane_s16(g16, 0);
    G[row  ][col+1] = (uint8_t)vget_lane_s16(g16, 1);
    G[row+1][col]   = (uint8_t)vget_lane_s16(g16, 2);
    G[row+1][col+1] = (uint8_t)vget_lane_s16(g16, 3);

    B[row  ][col]   = (uint8_t)vget_lane_s16(b16, 0);
    B[row  ][col+1] = (uint8_t)vget_lane_s16(b16, 1);
    B[row+1][col]   = (uint8_t)vget_lane_s16(b16, 2);
    B[row+1][col+1] = (uint8_t)vget_lane_s16(b16, 3);
}
